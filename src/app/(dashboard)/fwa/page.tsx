...
const attestationStats = [
  { label: 'Attestation Required',  value: '68,247', color: '#4F46E5', bg: '#EEF2FF', border: '#C7D2FE' },
  ...
  return (
    <>
      ...
      <main style={{ padding: '16px 20px 40px' }}>
        ...
        {scanDone && !sent && (
          <div style={{ marginBottom: 14, padding: '10px 16px', borderRadius: 10, background: '#EFF6FF', border: '1px solid #BFDBFE', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 600, color: '#2563EB' }}>
          ...
          <Card>
            ...
            <CardBody style={{ padding: '16px 20px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
                <span style={{ fontSize: 48, fontWeight: 800, color: '#4F46E5', lineHeight: 1 }}>{completionPct}%</span>
                <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>62,184 of 68,247 pharmacies</span>
              </div>
              <Progress value={completionPct} color="#4F46E5" height={10}/>
              ...
              <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ...
                  <div key={b.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-muted)', width: 36, textAlign: 'right' }}>{b.pct}%</span>
                  </div>
                ))}
              </div>
              ...
              {completionByNetwork.map(n => (
                <div key={n.network}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text-primary)' }}>{n.network}</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: n.pct >= 90 ? '#10B981' : '#D97706' }}>{n.pct}%</span>
                  </div>
                  <Progress value={n.pct} color={n.pct >= 90 ? '#10B981' : '#D97706'} height={6}/>
                  <div style={{ fontSize: 10.5, color: 'var(--text-muted)', marginTop: 3 }}>{n.count} attested</div>
                </div>
              ))}
            </CardBody>
          </Card>
          ...
          <Card>
            <CardHeader
              ...
              action={
                selected.length > 0 ? (
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button className="btn-primary" style={{ fontSize: 11, padding: '4px 12px', gap: 5 }} onClick={sendReminders} disabled={sending}>
                      <IconBell size={11} color="#fff"/> Send Reminders ({selected.length})
                    </button>
                  ...
                  {overdueList.map(r => (
                    <tr key={r.id} style={{ background: selected.includes(r.id) ? '#FFF5F5' : undefined }}>
                      ...
                      <td>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{r.daysOver} days overdue</div>
                      </td>
                      <td>
                        <span style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Card>
              ...
              <CardBody style={{ padding: '12px 16px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {riskBands.map(b => (
                  <div key={b.label} style={{ padding: '12px', borderRadius: 10, background: b.bg, border: `1px solid ${b.color}22` }}>
            ...
            <div style={{ padding: '12px 14px', borderRadius: 12, background: 'linear-gradient(135deg, rgba(239,68,68,.06), rgba(245,158,11,.06))', border: '1px solid rgba(239,68,68,.15)' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#DC2626', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 4 }}>Powered by</div>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--text-primary)' }}>AGT-11: FWA Risk Scoring</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, lineHeight: 1.4 }}>AI-powered fraud risk analysis across non-attested pharmacies</div>
            </div>
          </div>
        </div>
      </main>
